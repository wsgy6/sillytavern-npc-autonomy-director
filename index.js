...
function getActiveGoalPhase(role) {
  const goalChain = createGoalChainState(role?.goalChain);
  for (const phase of GOAL_PHASE_ORDER) {
    const layer = normalizeGoalLayerState(goalChain[phase]);
...
function getActiveGoalText(role, preferredPhase = '') {
  ...
  const targetGoal = getGoalLayer(role, targetPhase).goal;
  if (targetGoal) {
    return targetGoal;
  }
  for (const phase of GOAL_PHASE_ORDER) {
    const goal = getGoalLayer(role, phase).goal;
    if (goal) {
      return goal;
    }
  }
...
function syncRoleGoalState(role, preferredPhase = '') {
  const hasAnyGoal = GOAL_PHASE_ORDER.some(phase => Boolean(getGoalLayer(role, phase).goal));
...
function getGoalLayer(role, phase) {
  return getGoal(role, phase);
}

async function setGoal(role, phase, goal, completed = false, extraPatch = {}) {
...
async function generateGoal(role, phase) {
  const ids = getCurrentIdentifiers();
  const fallbackGoal = buildGoalFallback(role, ids, phase);
...
async function assignGoalToPhase(role, ids, phase, force = false) {
  ...
  const safeTask = (async () => {
    const fallbackGoal = buildGoalFallback(targetRole, ids, phase);

    try {
      const nextGoal = await generateGoal(targetRole, phase);
      const safeGoal = sanitizeGoalText(nextGoal) || fallbackGoal;
      return setGoal(targetRole, phase, safeGoal, false, {
        progressSignals: 0,
        completionLockedAt: '',
      });
    } catch (error) {
      console.warn('[NPC 自主导演] 分配目标失败，改用模板目标。', error?.message || error);
      return setGoal(targetRole, phase, fallbackGoal, false, {
        progressSignals: 0,
        completionLockedAt: '',
      });
    }
    ...
    return setGoal(targetRole, phase, buildGoalFallback(targetRole, ids, phase), false, {
      progressSignals: 0,
      completionLockedAt: '',
    });
  });
...
async function refreshGoalChain(role, ids, force = false) {
  ...
  nextRole = await assignGoalToPhase(nextRole, ids, 'late', force || !getGoal(nextRole, 'late').goal);
  nextRole = await assignGoalToPhase(nextRole, ids, 'middle', force || !getGoal(nextRole, 'middle').goal);
  nextRole = await assignGoalToPhase(nextRole, ids, 'early', force || !getGoal(nextRole, 'early').goal || getGoal(nextRole, 'early').completed);
...
async function updateGoalChainAfterAdvance(role, reason = 'auto') {
  ...
  nextRole = markGoalLayerProgress(nextRole, activePhase, reason === 'manual' ? '手动推进一次阶段目标。' : '根据行动推进当前阶段目标。', true);
  ...
  if (getGoal(nextRole, activePhase).completed) {
    ...
    if (currentIndex >= 0 && currentIndex < GOAL_PHASE_ORDER.length - 1) {
      ...
      nextRole = await assignGoalToPhase(nextRole, getCurrentIdentifiers(), nextPhase, false);
...
function applyNpcFeedback(role, feedback) {
  ...
  if (feedback?.goalSignal) {
    const activePhase = getActiveGoalPhase(nextRole);
    nextRole = markGoalLayerProgress(nextRole, activePhase, '行动文本命中了目标完成信号。', true);
  }
...
async function triggerGoalReevaluation(role, ids) {
  ...
  const completedGoalChain = createGoalChainState({
    ...nextRoleBase.goalChain,
    [activePhase]: {
      ...activeLayer,
      completed: true,
  ...
  if (currentIndex >= 0 && currentIndex < GOAL_PHASE_ORDER.length - 1) {
    const nextPhase = GOAL_PHASE_ORDER[currentIndex + 1];
    nextRole = await assignGoalToPhase(nextRole, ids, nextPhase, false);
    nextRole = syncRoleGoalState(nextRole, nextPhase);
  }
...
async function processNpcActionsAndFeedback(role, actionText, source = 'llm', reason = 'auto') {
  ...
  let nextRole = applyNpcFeedback(withAction, feedback);
  nextRole = await advanceRole(nextRole, reason);
  nextRole = await triggerGoalReevaluation(nextRole, getCurrentIdentifiers());
  return syncRoleGoalState(nextRole);
}
...
async function advanceRole(role, reason = 'auto') {
  ...
  if (Number(nextRole.turnsSinceGoal || 0) >= goalInterval) {
    ...
    nextRole = await refreshGoalChain(nextRole, getCurrentIdentifiers(), false);
  }
...
async function handleMessageReceived(messageId) {
  ...
  let state = await ensureRolesInitialized();
  let roles = state.roles.filter(role => role.enabled);
  if (!roles.length) return;

  const initialGoalFlagKey = `${MODULE_NAME}_initial_goal_generated`;
  const aiMessageCount = Array.isArray(ctx.chat)
    ? ctx.chat.filter(item => item && !item.is_user && !item.is_system).length
    : 0;
  const shouldGenerateInitialGoals = !ctx.chatMetadata?.[initialGoalFlagKey] && aiMessageCount === 1;

  if (shouldGenerateInitialGoals) {
    const ids = getCurrentIdentifiers();
    state = await updateState(async currentState => ({
      ...currentState,
      roles: await Promise.all(currentState.roles.map(async role => {
        if (!role.enabled) return role;
        let nextRole = await assignGoalToPhase(role, ids, 'late', true);
        nextRole = await assignGoalToPhase(nextRole, ids, 'middle', true);
        nextRole = await assignGoalToPhase(nextRole, ids, 'early', true);
        return nextRole;
      })),
    }));
    ctx.chatMetadata[initialGoalFlagKey] = true;
    persistMetadata();
    roles = state.roles.filter(role => role.enabled);
  }
